...
15. **Entity ids are DISTINCT types.** Every entity id is a distinct defined type in `internal/ids` (`ids.AccountID`, `ids.FundID`, …), never bare `int64`. A function that expects an account id takes `ids.AccountID`; one id type may never be passed where another is expected. Bare `int64` appears for an id ONLY at the two conversion boundaries — the SQL scan boundary (the DB hands back `int64`; wrap immediately when building a model or in the query func) and the HTTP string-parse boundary (`ids.AccountID(parseID(s))`) — and is converted immediately there. Non-id `int64` (money minor units, counts, positions, exponents, sort orders) stays `int64`. Nullable FKs stay `sql.NullInt64` at the scan boundary and become `*ids.XID` at the store boundary via `ids.Ptr`/`ids.Null`. Audit-row PKs (`*_versions.ID`) stay `int64`; a `*_versions.EntityID` is the typed entity id.
