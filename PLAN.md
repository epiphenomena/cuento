...
- [ ] **p25.4 ux: expense-report auto-row line grid.** Replace the one-at-a-time "New line" inline form with the same auto-row grid (account/amount/fund/program/memo — NO balancing, NO min-2, NO DR/CR, NO functional-class, just a line-id round-trip). Needs a bulk replace-set store method (diff-by-line-id + versioning, like `UpdateTransaction`'s split diff) to save the grid under one change, replacing the per-line Add/Update/Remove. Overturns the p20.2 "NO bulk-save grid, NO new ES module" design note (update that comment). This is the large/risky piece — do it with the same verify-and-commit rigor or flag it.
