...
- [x] **p12.6 ux: entry-flow hardening.**
  ...
  Follow-up (p12.2 gap surfaced in p12.6, UNWIRED): `txngrid.js`'s keyboard state machine (`nextCell`: Enter-advance, Alt+Arrow row-move, Ctrl+Enter save, Escape) is built + node-tested but **never called** — `txneditor.js` imports it but wires no grid keydown handler, so those shortcuts are inert in the browser (keyboard entry currently works via native Tab + native select + Add-row). Wiring it needs a skip-hidden traversal (`nextCell` blindly does col+1 and would land focus on the hidden program/class cells of non-R/E rows), which does not exist yet. AWAITING USER DECISION: wire the Enter/Alt-arrow/Ctrl+Enter ergonomics (p12.2 follow-up + skip-hidden fix + tests) vs. keep native-Tab entry and drop them.
