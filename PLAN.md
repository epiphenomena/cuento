...
- [ ] **p24.2 data: transaction-level notes.** Splits already carry short `memo`s; add a longer multiline `notes` free-text field to `transactions` (+ `transactions_versions`, + every version SELECT for Z3 snapshot parity), thread it through the sqlc queries / store `PostTransactionInput` / the editor handler + a `<textarea>` in the transaction form, and surface it in the history view. `txn.notes` catalog key (both locales).
- [ ] **p24.3 investigate: notes on other records.** Funds/budgets/budget_schedules/reconciliations ALREADY have `notes` columns — check whether they are surfaced in their edit forms or dormant; accounts/programs/payees have none. Come back with a recommendation (cheap template-surfacing vs new migrations) rather than guessing.
