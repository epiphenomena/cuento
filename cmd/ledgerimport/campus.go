...
//
//   Pass 1 (campusPlan) -- a GLOBAL, CHRONOLOGICAL drawdown pool. Restricted
//   revenue received is spendable only until it runs out. We walk every campus
//   revenue/expense split in DATE order (tie-broken by tid then group index for
//   determinism -- and so every per-subsidiary import process, each handed the
//   FULL export, derives the IDENTICAL decision), maintaining a running pool
//   balance PER CURRENCY (campus splits post in USD and HNL; adding minor units
//   across currencies would be nonsense). A campus REVENUE split always joins the
//   fund and grows the pool; a campus EXPENSE split joins the fund only if the pool
//   covers it (whole-split granularity) and draws the pool down, else it OVERFLOWS
//   to unrestricted (donor ignored -- an overspent campus expense is just an
//   ordinary unrestricted cost). Once the pool is exhausted, later campus expenses
//   stay unrestricted. The pool is asymmetric ON PURPOSE: revenue may leave the
//   fund positive (unexpended restricted resources), but expense can never push it
//   below zero.
//
...
func (b *builder) buildCampusPlan(groups map[string][]Record) (campusPlan, error) {
	plan := campusPlan{}
	if b.res.CampusFundID == nil {
		return plan, nil // feature off: no campus fund, no drawdown
	}

	// Collect every campus R/E split as a dated event carrying its stable key and
	// exact net-debit amount (rule 3: from db/cr, never the float-noisy columns).
	type event struct {
		date   string
		tid    string
		idx    int
		cur    string
		amount int64 // signed net-debit (revenue < 0, expense > 0)
	...
	pool := map[string]int64{} // currency -> restricted net assets available (>= 0)
	for _, e := range events {
		if e.isRev {
			// Revenue received is restricted until spent: always RtW, grows the pool by
			// its magnitude (a credit net-debit is negative, so subtract to add).
			pool[e.cur] -= e.amount
			plan[e.key] = true
			continue
		}
		// Expense (positive net-debit): RtW only while the pool covers the whole
		// split; else it overflows to unrestricted and the pool is left untouched.
		if pool[e.cur] >= e.amount {
			pool[e.cur] -= e.amount
			plan[e.key] = true
		} else {
			plan[e.key] = false
