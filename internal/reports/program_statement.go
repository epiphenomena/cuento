...
//
// CONVERSION: NONE — the statement reads in NATIVE currency, per-currency rows (like the
// sibling activity reports p15.8/p15.9). A program is a management/mission dimension read
// in the money each grant/spend occurred in; converting would blur that and the 990
// preparer wants the native figures. Documented in DECISIONS.
//
...
func registerProgramStatement(reg *Registry) {
	reg.Register(Report{
		...
		Group:      "programs",
		ParamsSpec: ParamsSpec{Period: true, Program: true},
...
func runProgramStatement(ctx context.Context, tk *Toolkit, p Params) (Table, error) {
	...
	// Rolled per-(program, account) native activity: a parent program's cells fold in its
	// descendants (ProgramActivity does the tree rollup, D24). RateNone => native per
	// currency, exactly the figures the per-currency rows show and the drills reconcile to.
	act, err := tk.ProgramActivity(ctx, scope, p.From, p.To, ConvertOpts{Mode: RateNone})
	if err != nil {
		return Table{}, err
	}
	...
	b := &psBuilder{tk: tk, p: p, cols: cols, act: act}
	b.columns()
...
func (b *psBuilder) columns() {
	b.tableCols = append(
		b.tableCols,
		Column{HeaderKey: "reports.program_statement.col.account", Align: AlignLeft},
		Column{HeaderKey: "reports.program_statement.col.currency", Align: AlignLeft},
	)
...
func (b *psBuilder) section(tree []treeNode, typ, sectionKey, totalKey string, sign int64) map[string][]int64 {
	...
	for _, acct := range order {
		...
		for _, ccy := range ccys {
			...
			cells = append(cells, TextCell(name[acct]), TextCell(ccy))
			for col, c := range b.cols {
				raw := byCcy[ccy][col]
				addSum(sectionSum, ccy, col, raw)
				cell := MoneyCell(sign*raw, ccy)
...
func (b *psBuilder) headerRow(key string) {
	cells := make([]Cell, 0, len(b.cols)+2)
	cells = append(cells, LabelCell(key), TextCell(""))
	for range b.cols {
...
func (b *psBuilder) totalRows(key string, sums map[string][]int64, sign int64) {
	...
	for _, ccy := range ccys {
		cells := make([]Cell, 0, len(b.cols)+2)
		cells = append(cells, LabelCell(key), TextCell(ccy))
		for col := range b.cols {
			cells = append(cells, MoneyCell(sign*sums[ccy][col], ccy))
		}
...
func (b *psBuilder) netLines(revNet, expNet map[string][]int64) {
	...
	for _, ccy := range ccys {
		cells := make([]Cell, 0, len(b.cols)+2)
		cells = append(cells, LabelCell("reports.program_statement.net"), TextCell(ccy))
		for col := range b.cols {
			var rev, exp int64
			if revNet[ccy] != nil {
				rev = revNet[ccy][col]
			...
			cells = append(cells, MoneyCell(-(rev+exp), ccy))
		}
