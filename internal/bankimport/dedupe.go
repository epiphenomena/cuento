...
//
// Formula (PLAN p17.2): sha256(account | date | amount | normalized(payee+memo)),
// the four parts joined by '|'. Concretely:
//
...
//	payeememo= Normalize(payee) + "\x1f" + Normalize(memo) -- payee and memo are
//	           each normalized SEPARATELY, then joined by a unit separator, so
//	           "AB"+"C" and "A"+"BC" never alias AND whitespace adjacent to the
//	           separator cannot leak a stray space into the boundary.
//
// The '\x1f' unit separator between the normalized payee and memo and the '|'
...
const (
	...
	fieldSep = "|"
	// pairSep joins payee and memo before normalization (a byte that will not
...
// for the same (account, date, amount, payee/memo) event.
func DedupeHash(accountID int64, date string, amountMinor int64, payee, memo string) string {
	key := strings.Join([]string{
		strconv.FormatInt(accountID, 10),
		date,
		strconv.FormatInt(amountMinor, 10),
		Normalize(payee) + pairSep + Normalize(memo),
	}, fieldSep)
...
// payee and the memo SEPARATELY (DedupeHash joins the two normalized results with
// a unit separator), so trailing/leading whitespace on either can never leak a
