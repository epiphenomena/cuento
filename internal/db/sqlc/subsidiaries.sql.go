...
func (q *Queries) CountActiveChildren(ctx context.Context, parentID sql.NullInt64) (int64, error) {
	row := q.db.QueryRowContext(ctx, countActiveChildren, parentID)
	var count int64
	err := row.Scan(&count)
	return count, err
}
...
func (q *Queries) Descendants(ctx context.Context, id int64) ([]DescendantsRow, error) {
	...
	defer rows.Close()
	var items []DescendantsRow
	for rows.Next() {
		var i DescendantsRow
		if err := rows.Scan(
			&i.ID,
			&i.ParentID,
			&i.Name,
...
const insertSubsidiary = `-- name: InsertSubsidiary :one
...
type InsertSubsidiaryParams struct {
	ParentID     sql.NullInt64
	Name         string
	BaseCurrency string
	Active       int64
...
func (q *Queries) InsertSubsidiaryVersion(ctx context.Context, arg InsertSubsidiaryVersionParams) error {
	_, err := q.db.ExecContext(ctx, insertSubsidiaryVersion, arg.Op, arg.ID, arg.ID_2)
	return err
}
...
func (q *Queries) SubTree(ctx context.Context) ([]SubTreeRow, error) {
	rows, err := q.db.QueryContext(ctx, subTree)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
